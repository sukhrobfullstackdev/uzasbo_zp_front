...
const BillingListSendServices = {
  getSendList(pageNumber, pageLimit, sortColumn, orderType, search, date, filter, filterFormValues) {
    ...
    return ApiServices.get(
      `${baseUrl}GetIndexSend?SortColumn=${sortColumn ? sortColumn : ""}&OrderType=${orderType ? orderType : ""}&PageNumber=${pageNumber}&PageLimit=${pageLimit}&ID=${filter?.ID ? filter?.ID : ''}&Search=${filter?.Name ? filter?.Name : ''}&INN=${filter?.INN ? filter?.INN : ''}&Year=${date?.Year ? date?.Year : ''}`
    );
  },
...
export default BillingListSendServices;
