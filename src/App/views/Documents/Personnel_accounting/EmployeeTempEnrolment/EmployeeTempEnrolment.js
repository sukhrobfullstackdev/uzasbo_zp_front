...
class EmployeeMovement extends Component {
  ...
  fetch = (params = {}, filterFormValues) => {
    ...
    EmployeeTempEnrolmentServices.getList(pageNumber, pageLimit, sortColumn, orderType, date, filter, filterFormValues)
      .then((data) => {
        this.setState({
          loading: false,
          data: data.data.rows,
  ...
  acceptHandler = (id) => {
    ...
    EmployeeTempEnrolmentServices.Accept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  declineHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterData } = this.state;
    EmployeeTempEnrolmentServices.NotAccept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  deleteRowHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterData } = this.state;
    EmployeeTempEnrolmentServices.delete(id)
      .then((res) => {
        if (res.status === 200) {
          this.fetch({ pagination }, filterData);
        }
  ...
  render() {
    ...
    return (
      <Card title={t("EmployeeTempEnrolment")}>
        <Fade>
