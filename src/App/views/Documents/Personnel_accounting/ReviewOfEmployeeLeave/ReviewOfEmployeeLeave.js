...
class ReviewOfEmployeeLeave extends Component {
  ...
  fetch = (params = {}, filterFormValues) => {
    ...
    ReviewOfEmployeeLeaveServices.getList(pageNumber, pageLimit, sortColumn, orderType, date, filter, filterFormValues)
      .then((data) => {
        this.setState({
          loading: false,
          data: data.data.rows,
  ...
  deleteRowHandler = (id) => {
    ...
    ReviewOfEmployeeLeaveServices.delete(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  acceptHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterData } = this.state;
    ReviewOfEmployeeLeaveServices.Accept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  declineHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterData } = this.state;
    ReviewOfEmployeeLeaveServices.NotAccept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  render() {
    ...
    return (
      <Card title={t("ReviewOfEmployeeLeave")}>
        <Fade>
              ...
              <div className="main-table-filter-elements">
                ...
                >
                  ...
                  >
                    ...
                    <Option value="PersonnelNumber">{t('number')}</Option>
                ...
                <Form.Item
                  ...
                  >
                    {this.state.departmentList.map(item => <Option key={item.ID} value={item.ID}>
                      {item.ShortName}
                    </Option>)}
                  </Select>
                </Form.Item>
...
export default withTranslation()(ReviewOfEmployeeLeave);
