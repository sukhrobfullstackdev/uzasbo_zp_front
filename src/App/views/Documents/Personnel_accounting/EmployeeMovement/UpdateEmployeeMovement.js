...
const EmployeeMovement = (props) => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const EmployeeMovement = await EmployeeMovementServices.getById(docId);
        ...
        const minimalSalaryList = await HelperServices.GetMinimalSalary(EmployeeMovement.data.Date);
        ...
        setTableId(EmployeeMovement.data.TableID);
        ...
        form.setFieldsValue({
          ...
          minimalSalaryls: minimalSalaryList.data,
          PlanTotal:
            EmployeeMovement.data.PlanWorkLoad + +EmployeeMovement.data.PlanTeachingLoad,

          PlanTeaching:
            EmployeeMovement.data.WorkLoad + +EmployeeMovement.data.TeachingLoad,
        })
  ...
  const fillTableHandler = async (params) => {
    form.validateFields()
      .then((values) => {
        ...
        EmployeeMovementServices.postDataFillTableData(values)
          .then(res => {
            if (res.status === 200) {
  ...
  const calculationTableHandler = () => {
    form.validateFields()
      .then((values) => {
        ...
        setTableLoading(true);
        EmployeeMovementServices.postDataCalTableData(values)
          .then(res => {
  ...
  const saveAllHandler = () => {
    form.validateFields()
      .then((values) => {
        ...
        setLoader(true);
        EmployeeMovementServices.postData(formData)
          .then(res => {
            history.push('/EmployeeMovement');
  ...
  const tariffScaleHandler = (TariffScaleID, data) => {
    ...
    HelperServices.GetTariffScaleTableList(data['data-tariff'])
      .then(res => {
        ...
        //form.resetFields({TariffScaleID: null});
      })
      .catch(err => Notification('error', err))
  }
  ...
  // const setTeachingLoad = (e) => {
  //   let sum =  +form.getFieldValue('WorkLoad') + +form.getFieldValue('TeachingLoad');
  //   form.setFieldsValue({
  //     PlanTeaching: sum
  //   })
  // }
  ...
  const deleteFileHandler = () => {
    setLoader(true);
    CommonServices.deleteFile(currentDocId, tableId, 'employeemovement')
      .then(res => {
        if (res.status === 200) {
  ...
  const donwloadFileHandler = () => {
    setLoader(true);
    CommonServices.downloadFile(currentDocId, tableId, 'employeemovement')
      .then(res => {
        if (res.status === 200) {
          const url = window.URL.createObjectURL(new Blob([res.data]));
          const link = document.createElement("a");
          link.href = url;
          link.setAttribute("download", "employeemovement.pdf");
  ...
  const Tables = [
    {
      title: t('OrganizationsSettlementAccountID'),
  ...
  return (
    <Fade >
      <MainCard title={t("EmployeeMovement")} isOption EmployeeEnrolment = {mainCardData} minimalSalary = {minimalSalary}>
          ...
          >
            <Row gutter={[16, 16]}>
              ...
              {/* <Col xl={3} lg={8}>
                ...
                >
                  <InputNumber placeholder={t("Salary")}
                    disabled style={{ color: 'black', width: "100%" }}
                    formatter={value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ' ')}
                    decimalSeparator=','
                  />
                </Form.Item>
              ...
              {/* <Col xl={2} lg={12}>
                  <Form.Item
                    ...
                    <InputNumber disabled style={{ color: 'black', width: "100%" }}
                      formatter={value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ' ')}
                      decimalSeparator=','
                    />
                  </Form.Item>
              ...
              {showTariffScale === 3 && (<Col xl={4} lg={12}>
                ...
                >
                  <Input placeholder={t("PlanTotal")} value={"number2"} disabled
                    style={{ color: 'black' }} />
                </Form.Item>
              </Col>)}
              {showTariffScale === 3 && (<Col xl={4} lg={12}>
