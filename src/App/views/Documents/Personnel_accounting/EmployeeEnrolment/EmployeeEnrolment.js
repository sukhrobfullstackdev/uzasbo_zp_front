...
class EmployeeEnrolment extends Component {
  ...
  fetch = (params = {}, filterFormValues) => {
    ...
    EmployeeEnrolmentServices.getList(pageNumber, pageLimit, sortColumn, orderType, date, filter, filterFormValues)
      .then((data) => {
        this.setState({
          loading: false,
          data: data.data.rows,
  ...
  deleteRowHandler = (id) => {
    ...
    EmployeeEnrolmentServices.delete(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  acceptHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterData } = this.state;
    EmployeeEnrolmentServices.Accept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  declineHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterData } = this.state;
    EmployeeEnrolmentServices.NotAccept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  render() {
    ...
    return (
      <Card title={t("EmployeeEnrolment")}>
        <Fade>
...
export default withTranslation()(EmployeeEnrolment);
