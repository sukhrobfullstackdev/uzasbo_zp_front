...
class OrderOnLeaveOfAbsence extends Component {
  ...
  fetchData = async () => {
    try {
      ...
      this.setState({ statusList: stsList.data, departmentList: dpList.data })
    } catch (err) {
  ...
  fetch = (params = {}, searchCode, filterFormValues) => {
    ...
    OrderOnLeaveOfAbsenceServices.getList(pageNumber, pageLimit, sortColumn, orderType, search, date, filter, filterFormValues)
      .then((data) => {
        this.setState({
          loading: false,
          data: data.data.rows,
  ...
  deleteRowHandler = (id) => {
    ...
    OrderOnLeaveOfAbsenceServices.delete(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  acceptHandler = (id) => {
    ...
    OrderOnLeaveOfAbsenceServices.Accept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  declineHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterData } = this.state;
    OrderOnLeaveOfAbsenceServices.NotAccept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  render() {
    ...
    return (
      <Card title={t("OrderOnLeaveOfAbsence")}>
        <Fade>
...
export default withTranslation()(OrderOnLeaveOfAbsence);
