...
class StaffListRegisteryReceived extends Component {
    ...
    fetch = (params = {}, searchCode, filterFormValues) => {
        ...
        StaffListServices.getReceivedRegisteryList(pageNumber, pageLimit, sortColumn, orderType, search)
            .then((data) => {
                this.setState({
                    loading: false,
                    data: data.data.rows,
    ...
    acceptHandler = (id) => {
        this.setState({ loading: true });
        StaffListServices.ApproveStaffList(id)
            .then((res) => {
                if (res.status === 200) {
                    const { pagination, filterData } = this.state;
                    this.fetch({ pagination }, filterData);
    ...
    registerHandler = (id) => {
        this.setState({ loading: true });
        StaffListServices.RegisterStaffList(id)
            .then((res) => {
                if (res.status === 200) {
                    const { pagination, filterData } = this.state;
                    this.fetch({ pagination }, filterData);
    ...
    declineHandler = (id, tableId) => {
        confirm({
            ...
            onOk: () => {
                this.setState({ loading: true });
                StaffListServices.CancelReceivedRegistery(id, '', tableId)
                    .then((res) => {
                        if (res.status === 200) {
                            const { pagination, filterData } = this.state;
                            this.fetch({ pagination }, filterData);
    ...
    render() {
                ...
                <span onClick={() => this.showAcceptModal(record.ID)}>
                  <i className="far fa-paper-plane action-icon" />
                            ...
                            <Tooltip title={t("register")}>
                                <span onClick={() => this.registerHandler(record.ID)}>
                                    <i className="feather icon-check-square action-icon" />
                                </span>
                            </Tooltip>
                            <Tooltip title={t("Print")}>
                                <span onClick={() => this.PrintStaffList2(record.ID)}>
        ...
        return (
            <Card title={t("StaffListRegisteryReceived")} >
                <Fade>
                    <Form
                        layout='vertical'
                        ref={this.filterForm}
...
export default withTranslation()(StaffListRegisteryReceived);
