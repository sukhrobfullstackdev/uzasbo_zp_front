...
class StaffListRegisterySend extends Component {
    ...
    fetch = (params = {}, searchCode, filterFormValues) => {
        ...
        StaffListServices.getSentRegisteryList(pageNumber, pageLimit, sortColumn, orderType, search, date)
            .then((data) => {
                this.setState({
                    loading: false,
                    data: data.data.rows,
    ...
    acceptHandler = (id, tableId) => {
        this.setState({ loading: true });
        StaffListServices.ReceivedRegistery(id, tableId)
            .then((res) => {
                if (res.status === 200) {
                    const { pagination, filterData } = this.state;
                    this.fetch({ pagination }, filterData);
    ...
    declineHandler = (id, tableId) => {
        confirm({
            icon: <ExclamationCircleOutlined />,
            content: <div>
                <div>{this.props.t('Description')}</div>
                <Input
                    style={{ marginTop: 32 }}
                    name="Description"
            ...
            onOk: () => {
                this.setState({ loading: true });
                StaffListServices.CancelReceivedRegistery(id, this.state.description, tableId)
                    .then((res) => {
                        if (res.status === 200) {
                            const { pagination, filterData } = this.state;
                            this.fetch({ pagination }, filterData);
    ...
    onTableRow = (record) => {
        return {
            onDoubleClick: () => {
                this.props.history.push(`StaffList/view/${record.ID}`);
            },
            onContextMenu: event => {
    ...
    render() {
                ...
                <span onClick={() => this.showAcceptModal(record.ID)}>
                  <i className="far fa-paper-plane action-icon" />
                            ...
                            <Tooltip title={t("preview")}>
                                <Link to={`StaffList/view/${record.ID}`}>
                                    <i className='feather icon-eye action-icon' aria-hidden="true" />
                                </Link>
                            </Tooltip>
                            <Tooltip title={t("Accept")}>
                                <span onClick={() => this.acceptHandler(record.ID, record.TableID)}>
                                    <i className="far fa-check-circle action-icon" />
                                </span>
                            </Tooltip>
                            <Tooltip title={t("NotAccept")}>
                                <span onClick={() => this.declineHandler(record.ID, record.TableID)}>
                                    <i className="far fa-times-circle action-icon" />
                                </span>
                            </Tooltip>
                            <Tooltip title={t("Print")}>
                                <span onClick={() => this.printForm(record.ID, record.TableID)}>
        ...
        return (
            <Card title={t("StaffListRegisterySend")} >
                <Fade>
                    <Form
                        layout='vertical'
                        ref={this.filterForm}
...
export default withTranslation()(StaffListRegisterySend);
