...
const StaffTableHeader = (props) => {
    const [monthCountDisable, setMonthCountDisable] = useState(false);
    ...
    useEffect(() => {
        if (AllPositionsID) {
            const fetchData = async () => {
                const staffBasicPositionAmount = await StaffPositionAmountServices.GetStaffBasicPositionAmount({
                    AllPositionsID, PositionQualificationID, IsBasicByGroup,
                    StaffListGroupID: props.StaffListGroupID, Code: props.staffList.OrganizationFunctionalItemCode
                });
                setStaffBasicPositionAmount(staffBasicPositionAmount.data)
                setSalary(staffBasicPositionAmount.data)

                if (DriverID === 1678795) {
                    setFot(Math.floor(staffBasicPositionAmount.data * 1.228))
                    addStaffForm.setFieldsValue({
                        AllowanceType63: Math.floor(staffBasicPositionAmount.data * 0.228)
                    })
                } else {
                    setFot(staffBasicPositionAmount.data)
                }
    ...
    }, [AllPositionsID, PositionQualificationID])
    ...
    const salaryBlurHandler = e => {
        const orderNum = addStaffForm.getFieldValue('StaffQuantity');
        setFot(orderNum * e.target.value);
        setSalary(orderNum * e.target.value);
    }
    ...
    return (
        ...
        >
            <tr>
                ...
                {props.staffList?.SettlementAccountItemOfExpense !== "7092100075" && (
                    <>
                        {/* <th className='ant-table-cell'>
                            <Form.Item
                                label={t('Department')}
