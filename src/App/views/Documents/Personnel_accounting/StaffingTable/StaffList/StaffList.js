...
class StaffList extends Component {
  ...
  fetch = (params = {}, filterFormValues) => {
    ...
    StaffListServices.getList(pageNumber, pageLimit, sortColumn, orderType, search)
      .then((data) => {
        this.setState({
          loading: false,
          data: data.data.rows,
  ...
  declineHandler = (id) => {
    this.setState({ loading: true });
    StaffListServices.NotAccept(id)
      .then((res) => {
        if (res.status === 200) {
          const { pagination, filterData } = this.state;
          this.fetch({ pagination }, filterData);
  ...
  render() {
            ...
            <Space size="middle">
              <Tooltip title={t("preview")}>
                <Link to={`${this.props.match.path}/view/${record.ID}`}>
                  <i className='feather icon-eye action-icon' aria-hidden="true" />
                </Link>
              </Tooltip>
              {fullRecord.CanEdit === 1 && (
              ...
              <Tooltip title={t("Print")}>
                <span onClick={() => this.PrintStaffList(record.ID, record.TableID)}>
                  <i className="feather icon-printer action-icon" />
                </span>
              </Tooltip>
              {/* <Tooltip title={t("Рабочая форма")}>
                <span onClick={() => this.Print(record.ID)}>
                  <i className="feather icon-printer action-icon" />
                </span>
              </Tooltip> */}
              {fullRecord.CanDeleted === 1 && (
    ...
    const { data, pagination, loading, orgSettleAccId, staffListTypeID, protocolModalVisible, rowId, rowTableId } = this.state;

    if (orgSettleAccId && staffListTypeID) {
      addBtn = (
        <Link to={`${this.props.match.path}/add?OrganizationsSettlementAccountID=${orgSettleAccId}&StaffListTypeID=${staffListTypeID}`}
        >
          {t("add-new")}&nbsp;
      ...
      <Card title={t("StaffList")} >
        ...
        <Fade>
          <Table
            ...
            onRow={(record, fullRecord) => {
              if (fullRecord.CanEdit) {
                return {
                  onDoubleClick: () => {
