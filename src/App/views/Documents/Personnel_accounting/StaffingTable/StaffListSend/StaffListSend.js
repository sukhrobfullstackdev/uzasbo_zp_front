...
import { Table, Input, Form, Button, Select, Space, Popconfirm, Tooltip, Modal, Tag, DatePicker } from "antd";
...
class StaffListSend extends Component {
    ...
    fetch = (params = {}, searchCode, filterFormValues) => {
        ...
        StaffListServices.getSentList(pageNumber, pageLimit, sortColumn, orderType, search, date, filter, filterFormValues)
            .then((data) => {
                this.setState({
                    loading: false,
                    data: data.data.rows,
    ...
    declineHandler = (id) => {
        this.setState({ loading: true });
        StaffListServices.CancelReceived(id)
            .then((res) => {
                if (res.status === 200) {
                    const { pagination, filterData } = this.state;
                    this.fetch({ pagination }, filterData);
...
export default withTranslation()(StaffListSend);
