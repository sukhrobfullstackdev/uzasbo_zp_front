...
import ProtocolModal from "../StaffList/ProtocolModal";
...
class IndexStaffListRegistery extends Component {
    ...
    fetch = (params = {}, searchCode, filterFormValues) => {
        ...
        StaffListServices.getListRegistery(pageNumber, pageLimit, sortColumn, orderType, search)
            .then((data) => {
                this.setState({
                    loading: false,
                    data: data.data.rows,
    ...
    postLoadKey = (id, vo, reload) => {
        ...
        window.CAPIWS.callFunction({
            ...
            if (data.success) {
                ...
                StaffListServices.SendSignedStaffListRegistery(signData)
                    .then(res => {
                        if (res.status === 200) {
    ...
    onTableRow = (record) => {
        return {
            onDoubleClick: () => {
                this.props.history.push(`IndexStaffListRegistery/edit/${record.ID}`);
            },
            onContextMenu: event => {
                    ...
                    return (
                        <Space size="middle">
                            ...
                            <Tooltip title={t("Print")}>
                                <span onClick={() => this.PrintStaffList2(record.ID, record.TableID)}>
                                    <i className="feather icon-printer action-icon" />
                                </span>
                            </Tooltip>
                            <Tooltip title={t("Print")}>
                                <span onClick={() => this.printForm(record.ID)}>
                                    <i className="feather icon-printer action-icon" />
                                </span>
                            </Tooltip>
                            <Tooltip title={t("protocol")}>
                                <span onClick={this.openProtocolModalHandler}>
                                    <i className="feather icon-message-square action-icon" />
                                </span>
                            </Tooltip>
...
export default withTranslation()(IndexStaffListRegistery);
