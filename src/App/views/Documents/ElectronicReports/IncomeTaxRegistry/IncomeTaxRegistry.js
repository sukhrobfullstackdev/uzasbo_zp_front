...
import { Table, Input, Form, Button, Select, Space, Dropdown, Menu, Tag } from "antd";
...
class IncomeTaxRegistry extends Component {
  IncomeTaxSend = JSON.parse(localStorage.getItem('userInfo')).Roles.includes('IncomeTaxSend');
  state = {
    data: [],
    filterStatus: [],
    filterType: "",
  ...
  fetch = (params = {}, searchCode, filterFormValues) => {
    ...
    IncomeTaxRegistryServices.getList(
      pageNumber,
      pageLimit,
      sortColumn,
      orderType,
  ...
  sendHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterForm } = this.state;
    IncomeTaxRegistryServices.Send(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  render() {
    ...
    return (
      <Card title={t("IncomeTaxRegistry")}>
        <Fade>
