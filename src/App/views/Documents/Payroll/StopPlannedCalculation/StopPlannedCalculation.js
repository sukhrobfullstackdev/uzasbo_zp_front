...
class StopPlannedCalculation extends Component {
  ...
  fetch = (params = {}, filterFormValues) => {
    ...
    StopPlannedCalculationServices.getList(pageNumber, pageLimit, sortColumn, orderType, date, filter, filterFormValues)
      .then((data) => {
        this.setState({
          loading: false,
          data: data.data.rows,
  ...
  deleteRowHandler = (id) => {
    ...
    StopPlannedCalculationServices.delete(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  acceptHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterData } = this.state;
    StopPlannedCalculationServices.Accept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  declineHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterData } = this.state;
    StopPlannedCalculationServices.NotAccept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  render() {
    ...
    return (
      <Card title={t("StopPlanned Calculation")}>
        <Fade>
...
export default withTranslation()(StopPlannedCalculation);
