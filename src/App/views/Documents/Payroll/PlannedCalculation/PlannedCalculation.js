...
class PlannedCalculation extends Component {
  ...
  fetch = (params = {}, filterFormValues) => {
    ...
    PlannedCalculationServices.getList(pageNumber, pageLimit, sortColumn, orderType, date, filter, filterFormValues)
      .then((data) => {
        this.setState({
          loading: false,
          data: data.data.rows,
  ...
  deleteRowHandler = (id) => {
    ...
    PlannedCalculationServices.delete(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  acceptHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterData } = this.state;
    PlannedCalculationServices.Accept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  declineHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterData } = this.state;
    PlannedCalculationServices.NotAccept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  render() {
    ...
    return (
      <Card title={t("PlannedCalculation")}>
        <Fade>
...
export default withTranslation()(PlannedCalculation);
