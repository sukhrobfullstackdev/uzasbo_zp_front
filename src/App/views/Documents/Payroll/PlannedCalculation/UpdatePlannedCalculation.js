...
import PlannedCalculationServices from "../../../../../services/Documents/Payroll/PlannedCalculation/PlannedCalculation.services";
 ...
 // const { size } = useState([]);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const EmployeeMovement = await PlannedCalculationServices.getById(docId);
  ...
  const fetchTableData = (params = {}, filterValues, id) => {
    ...
    PlannedCalculationServices.getTableData(fillId, pageNumber, pageLimit, sortColumn, orderType, filterValues)
      .then((res) => {
        if (res.status === 200) {
  ...
  const fillTableHandler = async (values) => {
    ...
    setTableLoading(true);
 try {
   const mainData = await PlannedCalculationServices.postDataFillTableData(values, data)
  ...
  const clearRowsHandler = () => {
    setTableLoading(true);
    setTableData([]);
    PlannedCalculationServices.clearTable(docId)
  ...
  const deleteRowsHandler = () => {
    ...
    PlannedCalculationServices.deleteTableRow(selectedIds)
      .then(res => {
        if (res.status === 200) {
          setTableLoading(false)
          const { pagination } = tablePagination;
  ...
  //   setTableLoading(true)
  //   PlannedCalculationServices.postCalculatePayrollCharges(docId, values.IsByTimeSheet, values.TimeSheetStartDate, values.TimeSheetEndDate)
  //     .then(res => {
  //       const { pagination } = tablePagination;
  //       setTableData(res.data);
  //       fetchTableData({ pagination });
  ...
  const saveAllHandler = () => {
    form.validateFields()
      ...
      //  values.Tables = [...tableData, ...backendTableData]
        ...
        setLoader(true);
        PlannedCalculationServices.postData(values)
          .then(res => {
            history.push('/PlannedCalculation');
  ...
  return (
    <Fade >
      <MainCard title={t("PlannedCalculation")}>
