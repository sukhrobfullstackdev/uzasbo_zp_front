...
import IncomeInKindServices from "../../../../../services/Documents/Payroll/IncomeInKind/IncomeInKind";
...
class IncomeInKind extends Component {
  state = {
    data: [],
    departmentList: [],
    orgSettleAcc: [],
  ...
  fetch = (params = {}, filterFormValues) => {
    ...
    IncomeInKindServices.getList(pageNumber, pageLimit, sortColumn, orderType, date, filter, filterFormValues)
      .then((data) => {
        this.setState({
          loading: false,
          data: data.data.rows,
  ...
  deleteRowHandler = (id) => {
    ...
    IncomeInKindServices.delete(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  acceptHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterData } = this.state;
    IncomeInKindServices.Accept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  declineHandler = (id) => {
    this.setState({ loading: true });
    const { pagination, filterData } = this.state;
    IncomeInKindServices.NotAccept(id)
      .then((res) => {
        if (res.status === 200) {
...
export default withTranslation()(IncomeInKind);
