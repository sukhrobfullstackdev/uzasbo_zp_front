...
import CancelOrderOfScholarshipServices from "../../../../../services/Documents/StudentAccounting/CancelOrderOfScholarship/CancelOrderOfScholarship.services";
...
//main function
const EditCancelOrderOfScholarship = (props) => {
 ...
 // const [enrolmentTableModalVisible, setEnrolmentTableModalVisible] = useState(false);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const CancelOrderOfScholarship = await CancelOrderOfScholarshipServices.getById(docId);
        const SubCalculationKindList = await HelperServices.getAllSubCalculationKindList();
       // const departmentLs = await HelperServices.getDepartmentList(CancelOrderOfScholarship.data.DivisionID);
        //setDepartmentList(departmentLs.data);
        setSubCalculationKind(SubCalculationKindList.data.rows);
        setDepartmentList(CancelOrderOfScholarship.data)
        setTableData(CancelOrderOfScholarship.data.Tables)
       //console.log(currentDocId)
       // setQualificationCategoryId(CancelOrderOfScholarship.data.QualificationCategoryID)
       // divisionChangeHandler(CancelOrderOfScholarship.data.DivisionID);
        if (CancelOrderOfScholarship.data.StatusID === 2) {
          setDisabledActions(true);
        }
        setLoader(false);
        form.setFieldsValue({
          ...CancelOrderOfScholarship.data,
          Number: CancelOrderOfScholarship.data.OrderNumber,
          Date: moment(CancelOrderOfScholarship.data.Date, 'DD.MM.YYYY'),
          //   keyBeginDate: moment(CancelOrderOfScholarship.data.BeginDate, 'DD.MM.YYYY'),
          EndDate: moment(CancelOrderOfScholarship.data.EndDate, 'DD.MM.YYYY'),
          OrderDate: moment(CancelOrderOfScholarship.data.OrderDate, 'DD.MM.YYYY'),
         //Date: moment(CancelOrderOfScholarship.data.Date, 'DD.MM.YYYY'),
  ...
  const fetchTableData = (params = {}, id) => {
    ...
    CancelOrderOfScholarshipServices.getTableData(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  const fillTableHandler = async () => {
    form.validateFields()
      //.then((values) => {
        ...
        data.Recalc = 0;
        setTableLoading(true);
        CancelOrderOfScholarshipServices.postDataFillTableData(data)
          .then(res => {          
            if (res.status === 200) {
             // setTableData(data);
    ...
    form.validateFields()
      .then((values) => {
       ...
       // data.OrderNumber = data.OrderNumber;
        ...
        console.log(values)
        setLoader(true);
        CancelOrderOfScholarshipServices.postData(data)
          .then(res => {
            history.push('/CancelOrderOfScholarship');
  ...
  const deleteRowsHandler = () => {
    ...
    CancelOrderOfScholarshipServices.deleteTableRow(selectedIds)
      .then(res => {
        if (res.status === 200) {
          console.log(res)
          setTableLoading(false)
  ...
  return (
    <Fade >
      <MainCard title={t("CancelOrderOfScholarship")}>
...
export default EditCancelOrderOfScholarship;
