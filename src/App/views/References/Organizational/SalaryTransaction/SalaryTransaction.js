...
const SalaryTransaction = ({ match }) => {
    const { t } = useTranslation();
    const location = useLocation();
    const [filterForm] = Form.useForm();
    const dispatch = useDispatch();

    const tableList = useSelector((state) => state.SalTransList);
    ...
    return (
        <Card title={t("SalaryTransaction")}>
            <Fade>
                <Form
                    layout='vertical'
                    form={filterForm}
...
export default React.memo(SalaryTransaction);