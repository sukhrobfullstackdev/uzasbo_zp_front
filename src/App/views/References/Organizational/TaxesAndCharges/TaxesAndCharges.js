...
const TaxesAndCharges = ({ match }) => {
    const { t } = useTranslation();
    const [filterForm] = Form.useForm();
    const dispatch = useDispatch();

    const tableList = useSelector((state) => state.TaxesAndChargesList);
    ...
    return (
        <MainCard title={t("TaxesAndCharges")}>
            <Fade>
                <Form
                    layout='vertical'
                    form={filterForm}
...
export default React.memo(TaxesAndCharges);