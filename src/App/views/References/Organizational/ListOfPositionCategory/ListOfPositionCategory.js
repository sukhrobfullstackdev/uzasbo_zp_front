...
const ListOfPositionCategory = ({ match }) => {
    const { t } = useTranslation();
    const location = useLocation();
    const [filterForm] = Form.useForm();
    const dispatch = useDispatch();

    const tableList = useSelector((state) => state.PostCatList);
    ...
    return (
        <MainCard title={t("ListOfPositionCategory")}>
            <Fade>
                <Form
                    layout='vertical'
                    form={filterForm}
...
export default React.memo(ListOfPositionCategory);