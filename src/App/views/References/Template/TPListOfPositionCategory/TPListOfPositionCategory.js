...
import UpdateTPListOfPositionCategoryModal from './UpdateTPListOfPositionCategoryModal';
import { getListStartAction, setListFilter, setListFilterType } from './_redux/getListSlice';

const { Option } = Select;

const TPListOfPositionCategory = ({ match }) => {
    const { t } = useTranslation();
    const dispatch = useDispatch();
    const [filterForm] = Form.useForm();
    const adminViewRole = JSON.parse(localStorage.getItem('userInfo')).Roles.includes('ChangeUserEDS');

    const tpListOfPosCategoryList = useSelector((state) => state.tpListOfPosCategoryList);

    let tableData = tpListOfPosCategoryList.listSuccessData?.rows;
    let total = tpListOfPosCategoryList.listSuccessData?.total;
    let pagination = tpListOfPosCategoryList?.paginationData;
    let filter = tpListOfPosCategoryList?.filterData;
    ...
    const [filterType, setFilterType] = useState(tpListOfPosCategoryList.filterType);
    ...
    return (
        <Card title={t("TPListOfPositionCategory")}>
            <Fade>
                <Form
            ...
            <CSSTransition
                ...
                timeout={300}
            >
                <UpdateTPListOfPositionCategoryModal
                    visible={updateModal}
                    data={rowItem}
                    fetch={handleRefresh}
                    onCancel={() => {
...
export default React.memo(TPListOfPositionCategory);